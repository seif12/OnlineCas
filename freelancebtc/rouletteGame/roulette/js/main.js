...
$(document)
		.ready(
				function() {
					preloadImg('/rouletteGame/roulette/images/1k.png');
					preloadImg('/rouletteGame/roulette/images/10k.png');
					preloadImg('/rouletteGame/roulette/images/100.png');
					preloadImg('/rouletteGame/roulette/images/10.png');
					preloadImg('/rouletteGame/roulette/images/unmute.png');
					$('.chip-size').click(
							function() {
...
function spin() {
	...
	if (!spinningNow) {
		...
		$.ajax({
			'url' : '/roulette/roulette.asmx/SpinRoulette',
			'type' : 'POST',
			'data' : JSON.stringify({
				bets : validbets,
				idc : coin,
			...
			'success' : function(data) {
				...
				if (data['error'] != "") {
					...
					spin_circle(data['number'], data['amount']);
					parent.NewSeed(data.PreviousServerSeedSHA256,
							data.NextServerSeedSHA256, data.PreviousServerSeed,
							data.PreviousClientSeed);
				}
			},
			'error' : function() {
				showMessage("Failed to place bet.");
			}
		});
	}
}
